...
export async function updatePendixPendenciaCampos(
  ...
  let q = supabase
    ...
    .eq('id', id);
  if (eid) q = q.eq('escritorio_id', eid);
  const { data, error } = await q.select('*, pendix_clientes(nome, telefone)').single();
  if (error) throw error;
  return data as PendixPendencia;
}
...
export async function getPendixStats() {
  ...
  return {
    ...
    // Conta todas as empresas, não só as ativas — "Total de Clientes" ao lado
    // conta todos os clientes independente de status, então os dois cartões
    // "Total de X" precisam ter o mesmo critério pra não confundir o usuário.
